...
class hac():
	...
	def get_linkage(self):
		if self.hac_kwargs["distance_threshold"] is not None:
			...
			# hac take dens matrix as input of distance
			distance_matrix = pairwise_distances(self.X, metric=self.hac_kwargs["affinity"])
			dist_condens = squareform(distance_matrix)

			linkage_matrix = hierarchy.linkage(
					dist_condens, # distance_matrix, dist_condens
					method=self.hac_kwargs["linkage"], # ward, complete, single, average
					metric=self.hac_kwargs["affinity"])
	...
	def plot_dendrogram(self, saveat, **kwargs):
		if self.linkage_matrix is None:
			print ("We have not fit yet.")
			return 
		fig = plt.figure(figsize=(10, 10), dpi=300)
		# dendrogram(self.linkage_matrix, **kwargs)
		ddr = hierarchy.dendrogram(
...
def show_tree(model, ticklabels):
	...
	#     print (node_1, node_dict[node_1], node_2, node_dict[node_2])
			...
			G.add_edge(parent_node, node_1, weight= weight_n1) # 1 - np.log(weight_n1), distance, 30*weight_n1
			G.add_edge(parent_node, node_2, weight= 5*weight_n2) # 1 - np.log(weight_n2), distance
			parent_node += 1
	...
	nx.draw_networkx_labels(G, pos, labels = node_labels, width=weights, font_size=18)
