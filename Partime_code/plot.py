...
title_font = {'fontname': 'serif', 'size': 24}
size_point = 3
alpha_point = 0.3
n_neighbor = 3
...
def plot_density(values, save_at,title_name,  cmap_name="coolwarm", vmin=None, vmax=None):
	# input: matrix [n_rows, n_cols] of any value
	# output: figure
	fig = plt.figure(figsize=(10, 10))

	if vmin is None and vmax is None:
		max_abs = max((abs(np.min(values)), abs(np.max(values))))#######delete 0.95 value
		vmin = -max_abs
		vmax = max_abs
	...
	# img = cmap(values)
	# print (img)
	im = ax.imshow(values, cmap=cmap, interpolation='none', vmin=vmin, vmax=vmax)
	...
	save_file = "{0}.pdf".format(save_at)
...
def ax_histfill(x, label, ax, lbx, ubx, orientation="horizontal"):
	...
	y_plot, x_plot, patches = ax.hist(x, bins=1, histtype='bar', # step, stepfilled, 'bar', 'barstacked'
				density=True, label=label, log=False,  
	...
	# 	# print("_"*50)
	# 	# print(len(selected_reg))
	# 	# print(selected_reg)
	# 	for i in selected_reg:
	# 		# print(i)
	# 		# print("_"*10)
	# 		patches[i-1].set_color('orange')
